<?php
// ══════════════════════════════════════════════════════════════
// JMbenga Portfolio — Gestão de Mensagens (Inbox v2)
// ══════════════════════════════════════════════════════════════
...
if (in_array($filterStatus, ['new','read','replied','archived'])) {
    $where[]  = 'status_msg = ?';
    $params[] = $filterStatus;
...
if ($search !== '') {
    $where[]  = '(name_msg LIKE ? OR email_msg LIKE ? OR subject_msg LIKE ? OR message_msg LIKE ?)';
    $params[] = "%$search%"; $params[] = "%$search%";
    $params[] = "%$search%"; $params[] = "%$search%";
}

$sql = 'SELECT * FROM _contact_message';
...
// ── Estatísticas KPI ──────────────────────────────────────────
$totalAll      = (int) $db->query("SELECT COUNT(*) FROM _contact_message")->fetchColumn();
$totalNew      = (int) $db->query("SELECT COUNT(*) FROM _contact_message WHERE status_msg = 'new'")->fetchColumn();
$totalRead     = (int) $db->query("SELECT COUNT(*) FROM _contact_message WHERE status_msg = 'read'")->fetchColumn();
$totalReplied  = (int) $db->query("SELECT COUNT(*) FROM _contact_message WHERE status_msg = 'replied'")->fetchColumn();
$totalArchived = (int) $db->query("SELECT COUNT(*) FROM _contact_message WHERE status_msg = 'archived'")->fetchColumn();
$totalStarred  = (int) $db->query("SELECT COUNT(*) FROM _contact_message WHERE is_starred = 1")->fetchColumn();

// ── CSRF token para AJAX ──────────────────────────────────────
$csrfToken = $_SESSION['admin_csrf_token'];

// ── Helper: badge classe + ícone + label ──────────────────────
function msgBadge(string $status): array
{
    return match ($status) {
        'new'      => ['class' => 'badge-new',      'icon' => 'envelope',     'label' => 'Nova'],
        'read'     => ['class' => 'badge-read',     'icon' => 'check',        'label' => 'Lida'],
        'replied'  => ['class' => 'badge-replied',  'icon' => 'reply',        'label' => 'Respondida'],
...
/* ── Bulk actions bar (aparece quando há selecionados) ─────── */
.bulk-bar {
    display: none;
    align-items: center;
    gap: .75rem;
...
<body>
        ...
        <div class="content">
            ...
            <div class="kpi-row">
                ...
                <a href="?status=read" class="kpi-card <?= $filterStatus === 'read' ? 'kpi-active' : '' ?>">
                    ...
                    <div><span class="kpi-val"><?= $totalRead ?></span>
                        <div class="kpi-lbl">Lidas</div>
                    </div>
                </a>
                <a href="?status=replied" class="kpi-card <?= $filterStatus === 'replied' ? 'kpi-active' : '' ?>">
                    <span class="kpi-icon" style="background:rgba(16,185,129,.12);color:#10b981"><i
                            class="fas fa-reply"></i></span>
                    <div><span class="kpi-val"><?= $totalReplied ?></span>
                ...
                <a href="?status=starred" class="kpi-card <?= $filterStatus === 'starred' ? 'kpi-active' : '' ?>">
                    ...
                    <div><span class="kpi-val"><?= $totalStarred ?></span>
                        <div class="kpi-lbl">Favoritas</div>
                    </div>
                </a>
                <a href="?status=archived" class="kpi-card <?= $filterStatus === 'archived' ? 'kpi-active' : '' ?>">
                    ...
                    <div><span class="kpi-val"><?= $totalArchived ?></span>
                        <div class="kpi-lbl">Arquivadas</div>
                    </div>
                </a>
            </div>
            ...
            <form class="filter-bar" method="get">
                <div class="filter-search">
                    ...
                    <input type="text" name="search" placeholder="Pesquisar mensagens…" value="<?= e($search) ?>"
                        id="filterSearch">
                    <?php if ($search): ?>
                ...
                <select name="status" onchange="this.form.submit()" class="filter-select">
                    ...
                    <option value="replied" <?= $filterStatus === 'replied'  ? 'selected' : '' ?>>Respondidas</option>
                    <option value="archived" <?= $filterStatus === 'archived' ? 'selected' : '' ?>>Arquivadas</option>
                    <option value="starred" <?= $filterStatus === 'starred'  ? 'selected' : '' ?>>⭐ Favoritas</option>
                </select>
                ...
                <a href="<?= BASE_URL ?>/jm-panel/messages" class="btn btn-secondary" style="padding:.45rem .9rem">
                    <i class="fas fa-times"></i> Limpar
                </a>
                <span class="filter-result">
                    <i class="fas fa-circle-info"></i>
                    <?= count($messages) ?> resultado<?= count($messages) !== 1 ? 's' : '' ?>
                </span>
                <?php endif; ?>
            </form>
            ...
            <div class="table-card">
                ...
                <div class="empty-state">
                    ...
                    <p>
                        ...
                        Tenta ajustar os filtros ou <a href="<?= BASE_URL ?>/jm-panel/messages">ver todas</a>.
                        <?php else: ?>
                ...
                <table id="messagesTable" class="display responsive nowrap" style="width:100%">
                    ...
                    <tbody>
                        ...
                        $badge = msgBadge($m['status_msg']);
                    ?>
                        <tr id="row-<?= $m['id'] ?>" class="<?= $m['status_msg'] === 'new' ? 'row-new' : '' ?>">
                            <!-- Favorito -->
                            <td style="text-align:center">
                                <button class="btn-icon-star toggle-star" data-id="<?= $m['id'] ?>"
                                    data-starred="<?= $m['is_starred'] ?>"
                                    title="<?= $m['is_starred'] ? 'Remover favorito' : 'Marcar como favorito' ?>">
                                    <i class="fas fa-star"
                                        style="color:<?= $m['is_starred'] ? '#f59e0b' : 'var(--text-muted)' ?>"></i>
                                </button>
                            </td>
                            <!-- Remetente -->
                            <td>
                                <span class="msg-name"><?= e($m['name_msg']) ?></span>
                                <div style="font-size:.75rem;color:var(--text-muted)"><?= e($m['email_msg']) ?></div>
                                <?php if (!empty($m['phone_msg'])): ?>
                                <div class="msg-phone"><i class="fas fa-phone" style="font-size:.6rem"></i>
                                    <?= e($m['phone_msg']) ?></div>
                                <?php endif; ?>
                            </td>
                            <!-- Assunto -->
                            <td style="max-width:200px">
                                <span
                                    style="font-size:.83rem;font-weight:<?= $m['status_msg'] === 'new' ? '700' : '400' ?>">
                                    <?= e($m['subject_msg']) ?>
                                </span>
                            </td>
                            <!-- Preview -->
                            <td>
                                <div class="msg-preview">
                                    <?= e(mb_strlen($m['message_msg']) > 80 ? mb_substr($m['message_msg'], 0, 80) . '…' : $m['message_msg']) ?>
                                </div>
                            </td>
                            <!-- Estado -->
                            <td style="text-align:center">
                                ...
                                <?php if ($m['status_msg'] === 'replied' && !empty($m['replied_at'])): ?>
                                <span class="replied-time"><?= date('d/m H:i', strtotime($m['replied_at'])) ?></span>
                                <?php endif; ?>
                            </td>
                            <!-- Data -->
                            <td>
                                <span style="font-family:var(--font-mono);font-size:.74rem;color:var(--text-dim)"
                                    title="<?= date('d/m/Y H:i', strtotime($m['created_at'])) ?>">
                            ...
                            <td>
                                <div class="row-actions" style="justify-content:center">
                                    <a href="<?= BASE_URL ?>/jm-panel/messages/view?id=<?= $m['id'] ?>" class="btn-icon"
                                        title="Ver mensagem">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="<?= BASE_URL ?>/jm-panel/messages/reply?id=<?= $m['id'] ?>"
                                        class="btn-icon" title="Responder"
                                        style="<?= $m['status_msg'] === 'replied' ? 'color:var(--accent)' : '' ?>">
                                        <i class="fas fa-reply"></i>
                                    </a>
                                    <button class="btn-icon mark-read" data-id="<?= $m['id'] ?>"
                                        data-status="<?= $m['status_msg'] ?>"
                                        title="<?= $m['status_msg'] === 'new' ? 'Marcar como lida' : 'Marcar como não lida' ?>">
                                        <i class="fas fa-<?= $m['status_msg'] === 'new' ? 'check' : 'envelope' ?>"></i>
                                    </button>
                                    <button class="btn-icon btn-icon-warning archive-message" data-id="<?= $m['id'] ?>"
                                        data-status="<?= $m['status_msg'] ?>"
                                        title="<?= $m['status_msg'] === 'archived' ? 'Restaurar' : 'Arquivar' ?>"
                                        style="<?= $m['status_msg'] === 'archived' ? 'color:#f59e0b' : '' ?>">
                                        <i
                                            class="fas fa-<?= $m['status_msg'] === 'archived' ? 'rotate-left' : 'box-archive' ?>"></i>
                                    </button>
                                    <button class="btn-icon btn-icon-danger delete-message" data-id="<?= $m['id'] ?>"
                                        data-title="<?= e($m['name_msg']) ?>" title="Eliminar">
    ...
    document.querySelectorAll('.toggle-star').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            try {
                const res = await fetch(BASE + '/jm-panel/messages/toggle-star?id=' + id, {
                    method: 'POST',
                    headers: {
    ...
    document.querySelectorAll('.mark-read').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            try {
                const res = await fetch(BASE + '/jm-panel/messages/toggle-read?id=' + id, {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'X-CSRF-Token': CSRF
                ...
                if (data.success) {
                    ...
                    const labels = {
                        ...
                        replied: ['badge-replied', 'reply', 'Respondida'],
                    ...
                    toast('success', newStatus === 'read' ? 'Marcada como lida' :
                        'Marcada como não lida', 2000);
                } else {
    ...
    document.querySelectorAll('.archive-message').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            const endpoint = isArchived ?
                BASE + '/jm-panel/messages/unarchive?id=' + id :
                BASE + '/jm-panel/messages/archive?id=' + id;
            try {
                ...
                if (data.success) {
                    if (isArchived) {
                        // Restaurou: actualizar badge para 'read'
                        const badge = row.querySelector('.badge-status');
                        ...
                        toast('success', 'Mensagem restaurada.', 2000);
                    } else {
                        // Arquivou: remover da vista se não estamos no filtro "archived"
                        ...
                        toast('success', 'Mensagem arquivada.', 2000);
                    }
                } else {
    ...
    document.querySelectorAll('.delete-message').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (typeof Swal === 'undefined') {
                if (!confirm('Eliminar mensagem de "' + title + '"?')) return;
                doDelete(id);
                return;
            }
            Swal.fire({
    ...
    async function doDelete(id) {
        try {
            const res = await fetch(BASE + '/jm-panel/messages/delete?id=' + id, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'X-CSRF-Token': CSRF
