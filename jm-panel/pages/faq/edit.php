...
// ── Verificar se o ID foi passado e a FAQ existe ────────────
$editId = (int)($_GET['id'] ?? 0);
if ($editId <= 0) {
    redirect('/jm-panel/faq');
}
...
if (!$faq) {
    redirect('/jm-panel/faq?msg=notfound');
}
...
// Se não houve erro, usar dados da FAQ
if (empty($formData)) {
    $formData = $faq;
}
?>
        ...
        <div class="content">
            ...
            <div class="page-top">
                <div>
                    ...
                    <p class="page-sub">Altera os campos abaixo para actualizar a FAQ.</p>
                </div>
                <div style="display:flex;gap:.6rem;align-items:center">
                    ...
                    <button type="submit" form="faqForm" class="btn btn-primary" id="submitBtn">
                        <i class="fas fa-save"></i> Actualizar FAQ
                    </button>
                </div>
            </div>
            ...
            <!-- ═══ FORMULÁRIO ════════════════════════════════════════════ -->
            <form id="faqForm" novalidate data-api="<?= BASE_URL ?>/jm-panel/faq/edit-process">

                <input type="hidden" name="csrf_token" id="csrf_token" value="<?= $_SESSION['admin_csrf_token'] ?>">
                <input type="hidden" name="id_faq" value="<?= $editId ?>">
        ...
        try {
            ...
            if (!res.ok || !data.success) {
                if (data.errors) {
                    ...
                    showToast(data.message || 'Erro ao actualizar FAQ.', 'error');
                }
                submitBtns.forEach(btn => {
                    btn.disabled = false;
                    btn.innerHTML = '<i class="fas fa-save"></i> Actualizar FAQ';
                });
                return;
            }

            showToast('FAQ actualizada com sucesso!', 'success');
            setTimeout(() => {
                window.location.href = BASE_URL + '/jm-panel/faq?msg=updated';
            }, 800);
